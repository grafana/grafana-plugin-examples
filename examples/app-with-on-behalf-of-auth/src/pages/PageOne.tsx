...
export function PageOne() {
  ...
  const { error, loading, value } = useAsync(() => {
    return Promise.all([backendSrv.get(`api/plugins/grafana-appwithonbehalfofauth-app/health`)]);
  });
  ...
  const apiRequest = () => {
    ...
    backendSrv
      .get(`api/plugins/grafana-appwithonbehalfofauth-app/resources/api${apiPath}`, params)
      .then((response) => {
      ...
      <VerticalGroup>
        ...
        <div className={s.input}>
          <HorizontalGroup>
            ...
            <Checkbox value={onBehalfRequest} onChange={(e) => setOnBehalfRequest(e.currentTarget.checked)}></Checkbox>
            {onBehalfRequest && <Label>User ID</Label>}
            {onBehalfRequest && (
              <Input value={userID} onChange={(e) => setUserID(e.currentTarget.valueAsNumber)} type="number" />
            )}
